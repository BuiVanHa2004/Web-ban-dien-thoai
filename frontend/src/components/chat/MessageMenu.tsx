...
/* MessageWrapper                                                       */
/* Cấu trúc:                                                           */
/*  <hàng flex justify-end/start>                                      */
/*    <wrapper relative (chứa bubble + nút 3 chấm absolute)>           */
/*  </hàng>                                                             */
/*  [action bar bên dưới, cùng phía, nếu menu đang mở]                */
/* ------------------------------------------------------------------ */
interface MessageWrapperProps {
    ...
    children: React.ReactNode; // nội dung bubble
}
...
export function MessageWrapper({
    ...
    return (
        /* Toàn bộ khối (bubble + action bar) căn phải/trái */
        <div
            ref={wrapperRef}
            className={`flex flex-col ${isOwnMessage ? 'items-end' : 'items-start'}`}
        >
            {/* Hàng chứa bubble — căn phải/trái qua flex */}
            <div className={`flex w-full ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
                {/* Wrapper relative: bubble + nút 3 chấm nổi ra ngoài phía đối diện */}
                <div className="relative group">
                    {/* Nút 3 chấm — absolute, nằm NGOÀI bubble (margin âm) */}
                    <button
                        ...
                        className={`
                            ...
                            transition-opacity shadow hover:bg-black/70
                            ${isOwnMessage
                                ? '-left-8'   /* bên trái bubble (tin nhắn mình gửi ở phải) */
                                : '-right-8'  /* bên phải bubble (tin nhắn đối phương ở trái) */
                            }
                        `}
                        title="Thao tác"
                    >
            ...
            {/* Action bar — hiện NGAY BÊN DƯỚI bubble, không đè */}
            {showMenu && (
                <div
                    className={`
                        mt-1 bg-gray-900/95 backdrop-blur-xl rounded-full
                        shadow-2xl border border-gray-700 px-1.5 py-1.5 flex gap-1
...
/* Giữ default export để tránh lỗi import cũ */
