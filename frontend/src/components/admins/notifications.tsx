...
export default function NotificationsDropdown({ variant = "header" }: NotificationsDropdownProps) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const [resList, resCount] = await Promise.all([
        fetch(`${API_URL}/api/admin/notifications?size=10`, {
          headers: { 
  ...
  const handleNotificationClick = (notif: Notification) => {
    ...
    } else if (notif.type === "CONTACT" && notif.contactId) {
      router.push(`/contacts/${notif.contactId}`);
    } else if (notif.type === "EVALUATE") {
      if (notif.productId) {
        router.push(`/evaluates/${notif.productId}?evaluateId=${notif.evaluateId}`);
      } else {
        router.push(`/evaluates`);
      }
    }
  };
