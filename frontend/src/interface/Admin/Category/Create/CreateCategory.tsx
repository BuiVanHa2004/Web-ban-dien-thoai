    ...
    window.setTimeout(async () => {
      try {
        ...
        const uploadedUrls = await Promise.all(
          imageItems.map(item => fileUploadService.uploadCategoryImage(item.file))
        );
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3">
        <div>
          ...
          <h1 className="mt-3 text-xl font-semibold text-slate-900 dark:text-slate-100">Thêm danh mục</h1>
          <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">Tạo danh mục mới cho hệ thống.</p>
        </div>
      </div>

      <AdminActionBar backHref="/categories" formId="category-form" submitting={submitting} />
