...
export default function Trash() {
  ...
  async function refresh() {
    setError(null);
    try {
      const trashed = await orderService.getTrash();
      setItems(
        trashed.map<TrashItem>((o) => ({
          id: String(o.orderId),
          title: String(o.orderCode || o.orderId),
          subtitle: [o.customerName, o.productName].filter(Boolean).join(" - "),
          imageUrl: "",
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Thùng rác đơn hàng</h1>
          <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">
            Khôi phục hoặc xóa vĩnh viễn các đơn hàng đã xóa mềm.
          </p>
        </div>
        <Link
          href="/orders"
      ...
      <TrashView
        title="Thùng rác"
        description="Các đơn hàng đã bị xóa mềm sẽ nằm ở đây."
        items={items}
        onRestore={onRestore}
