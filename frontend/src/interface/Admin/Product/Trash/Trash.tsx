...
export default function Trash() {
  ...
  async function refresh() {
    setError(null);
    try {
      const trashed = await productService.getTrash();
      setItems(
        trashed.map<TrashItem>((p) => ({
          id: String(p.productId),
          title: p.productName,
          subtitle: p.productDescribe || "",
  ...
  async function onRestore(id: string) {
    try {
      await productService.restore(Number(id));
      await refresh();
    } catch (e: any) {
  ...
  async function onDeleteForever(id: string) {
    try {
      await productService.deleteForever(Number(id));
      await refresh();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Thùng rác sản phẩm</h1>
          <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">
            Khôi phục hoặc xóa vĩnh viễn các sản phẩm đã xóa mềm.
          </p>
        </div>
        <Link
          href="/products"
      ...
      <TrashView
        title="Thùng rác"
        description="Các sản phẩm đã bị xóa mềm sẽ nằm ở đây."
        items={items}
        onRestore={onRestore}
