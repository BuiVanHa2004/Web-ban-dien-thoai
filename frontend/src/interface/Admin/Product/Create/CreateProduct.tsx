...
import { productService, DiscountType, ProductType, ProductColorUpsertRequest } from "@/services/productService";
...
export default function CreateProduct() {
  ...
  const closeVariantsModal = React.useCallback(() => {
    ...
    setVariantsModalOpen(false);
  }, [variantsModalMode, variantsModalColorKey]);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [variantsModalOpen, closeVariantsModal]);
  ...
  function addColor() {
    setProductColors((prev) => {
      ...
      return [...prev, { key: nextKey, colorName: "", quantity: 0, variants: [createEmptyVariantInput()], images: [] }];
    });
  }
  ...
  function removeColorImage(colorKey: string, imageKey: string) {
    setProductColors((prev) => {
      const next = prev
        .map((c) => {
          if (c.key !== colorKey) return c;
          const removed = c.images.find((i) => i.key === imageKey);
          if (removed && removed.type === "new") {
            try {
              URL.revokeObjectURL(removed.previewUrl);
            } catch {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    window.setTimeout(async () => {
      try {
        ...
        const uploadedColorImagesByKey = new Map<string, Map<string, string>>();
        for (const c of productColors) {
          const m = new Map<string, string>();
          for (const img of c.images) {
            if (img.type === "new") {
              const uploaded = await productService.uploadProductImage(img.file);
              m.set(img.key, uploaded.url);
            } else {
              m.set(img.key, img.url);
            }
          }
          uploadedColorImagesByKey.set(c.key, m);
        }

        const payloadColors = productColors.reduce<ProductColorUpsertRequest[]>((acc, c) => {
          const colorName = c.colorName.trim();
          if (!colorName) return acc;

          const m = uploadedColorImagesByKey.get(c.key) || new Map<string, string>();
          const images = c.images
            .map((img) => m.get(img.key) || "")
            .filter((u) => !!u);
        ...
        const derived = isVariantCategory ? deriveVariantSpecValues(productColors) : null;
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-xl font-semibold text-slate-900 dark:text-slate-100">Thêm sản phẩm</h1>
          <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">Tạo sản phẩm mới.</p>
        </div>
      </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-2xl bg-slate-100 px-3 text-sm text-slate-900 ring-1 ring-slate-200 outline-none transition focus:ring-cyan-400/30 disabled:opacity-70 dark:bg-white/5 dark:text-slate-100 dark:ring-white/10"
            ...
            <div className="space-y-2">
              ...
              {thumbnailImage ? (
                <div className="pt-2">
                  ...
                  >
                    <Image
                      src={resolveImageUrl(thumbnailImage.previewUrl) || "https://dummyimage.com/200x356/e2e8f0/64748b&text=No+Image"}
              ...
              <div className="rounded-3xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/5">
                <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">Thêm màu nhanh</div>
                <div className="mt-3 space-y-3">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-slate-800 dark:text-slate-200">Biến thể (RAM/Bộ nhớ/Số lượng/Giá)</label>
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/5">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex cursor-pointer h-9 items-center justify-center rounded-2xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-800 shadow-sm transition hover:bg-slate-50 active:scale-95 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:hover:bg-white/10 whitespace-nowrap"
                        >
                          Nhập biến thể
                        </button>
                      </div>
                ...
                <div className="flex items-start gap-3">
                  <div className="h-24 w-14 shrink-0 overflow-hidden rounded-xl bg-white ring-1 ring-slate-200 dark:bg-white/5 dark:ring-white/10">
                    <Image
                      alt="preview"
                      src={
                        resolveImageUrl(primaryPreviewUrl) ||
                        "https://dummyimage.com/144x256/e2e8f0/64748b&text=Product"
                      }
                      width={56}
                      height={96}
                      unoptimized
