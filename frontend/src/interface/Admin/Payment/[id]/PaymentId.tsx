  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-3 flex items-center gap-4">
            <button
              id="btn-back-to-list"
              onClick={() => router.back()}
              className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:ring-slate-700 transition-all"
              aria-label="Quay lại trang danh sách"
            >
              <ArrowLeft className="h-5 w-5" />
            </button>
            <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100 tracking-tight">Thanh toán {order.orderCode}</h1>
          </div>
        ...
        <div className={`rounded-full px-4 py-2 text-xs font-black uppercase tracking-widest ${
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="overflow-hidden rounded-[2.5rem] border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900">
            <div className="border-b border-slate-100 p-6 dark:border-slate-800">
              <h2 className="flex items-center gap-2 text-lg font-black text-slate-900 dark:text-white">
                <ImageIcon className="h-5 w-5 text-indigo-500" /> Minh chứng chuyển khoản
              </h2>
            </div>
            <div className="p-8">
              <div className="relative aspect-[9/16] w-full max-w-sm mx-auto overflow-hidden rounded-3xl border-8 border-slate-50 bg-slate-50 dark:border-slate-800 dark:bg-slate-800">
                {paymentInfo?.latestAttempt?.transferImageUrl ? (
                  ...
                  <div className="flex h-full flex-col items-center justify-center text-slate-400">
                    ...
                    <p className="font-bold">Khách hàng chưa tải ảnh minh chứng</p>
                  </div>
                )}
              </div>
              ...
              {paymentInfo?.latestAttempt && (
                <div className="mt-8 grid gap-4 sm:grid-cols-2">
                  <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800">
                    ...
                    <p className="mt-1 text-sm font-medium italic">"{paymentInfo.latestAttempt?.transferNote || "Không có ghi chú"}"</p>
                  </div>
                  <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-800">
              ...
              {order.items?.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-center gap-4 p-6">
                    <div className="aspect-[9/16] w-20 flex-shrink-0 overflow-hidden rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                      <img
                        src={resolvedImg || ""}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap gap-2">
                        <span className="inline-flex items-center rounded-lg bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
                          {item.colorName}
                        </span>
                        <span className="inline-flex items-center rounded-lg bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                          Số lượng: {item.quantity}
                        </span>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-indigo-600">{formatVnd(Number(item.productPrice) * (item.quantity || 0))}</p>
                    </div>
                  </div>
                );
              })}
            </div>
            <div className="bg-slate-50 p-6 text-right dark:bg-slate-800/50">
              <span className="text-sm font-bold text-slate-500 mr-4">Tổng tiền thanh toán:</span>
              <span className="text-2xl font-black text-indigo-600">{formatVnd(order.totalAmount || 0)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {/* Order Status Card */}
          <div className="rounded-[2rem] bg-indigo-600 p-8 text-white shadow-xl shadow-indigo-500/30">
            <h3 className="text-xs font-black uppercase tracking-widest opacity-70">Trạng thái đơn hàng</h3>
            <div className="mt-2 text-2xl font-black">{translatePaymentStatus(order.orderStatus)}</div>
            <div className="mt-6 space-y-4">
              <div className="flex items-center justify-between text-sm">
                <span className="opacity-70">Thanh toán</span>
                <span className="font-black">{translatePaymentStatus(order.paymentStatus)}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="opacity-70">Phương thức</span>
                <span className="font-black">{translatePaymentStatus(order.paymentMethod)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
            <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-6">Thông tin khách hàng</h3>
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="rounded-xl bg-slate-100 p-2 dark:bg-slate-800"><User className="h-4 w-4" /></div>
                <div>
                  <p className="text-[10px] font-black uppercase text-slate-400">Người nhận</p>
                  <p className="text-sm font-bold">{order.receiverName || order.customerName}</p>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="rounded-xl bg-slate-100 p-2 dark:bg-slate-800"><Phone className="h-4 w-4" /></div>
                <div>
              ...
              <div className="flex items-start gap-4">
                <div className="rounded-xl bg-slate-100 p-2 dark:bg-slate-800"><MapPin className="h-4 w-4" /></div>
                <div>
                  <p className="text-[10px] font-black uppercase text-slate-400">Địa chỉ giao hàng</p>
                  <p className="text-xs font-medium leading-relaxed">{order.shippingAddress}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {order.comment && (
            <div className="rounded-[2rem] bg-indigo-50 p-8 shadow-sm ring-1 ring-indigo-200 dark:bg-indigo-900/10 dark:ring-indigo-500/20">
              <h3 className="text-xs font-black uppercase tracking-widest text-indigo-600 mb-4">Nội dung khách hàng nhập</h3>
          ...
          {/* Admin Note Card */}
          {order.adminNote && (
            <div className="rounded-[2rem] bg-amber-50 p-8 shadow-sm ring-1 ring-amber-200 dark:bg-amber-900/10 dark:ring-amber-500/20">
              <h3 className="text-xs font-black uppercase tracking-widest text-amber-600 mb-4">Ghi chú - {order.adminNoteAuthor || "Hệ thống"}</h3>
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300 italic">
                "{order.adminNote}"
              </p>
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {!isEditingNote ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-amber-500 px-3 py-1.5 text-[10px] font-black text-white hover:bg-amber-600 transition shadow-sm"
                >
                  SỬA GHI CHÚ
                </button>
              ) : (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-1.5 text-[10px] font-black text-white hover:bg-indigo-700 transition shadow-sm"
                >
          ...
          {(() => {
            ...
            return (
              <div className="space-y-6">
                <div className="rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                  <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-6">Kết quả đối soát</h3>
                  {effectiveStatus === "MATCHED" ? (
                    <div className="flex flex-col items-center justify-center py-6 gap-4 text-emerald-500">
                      <div className="rounded-full bg-emerald-100 p-4 dark:bg-emerald-900/30">
                        <Check className="h-8 w-8" />
                      </div>
                      <div className="text-2xl font-black uppercase tracking-widest">Đã thanh toán</div>
                      {(txAdminName || order.adminNoteAuthor) && (
                        <div className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-emerald-50/50 border border-emerald-100/50">
                          ...
                          <span className="text-[10px] font-black text-indigo-600">{txAdminName || order.adminNoteAuthor}</span>
                        </div>
                      )}
                      <p className="text-xs font-bold text-slate-500 text-center">Giao dịch đã được xác nhận khớp với đơn hàng.</p>
                    </div>
                  ) : effectiveStatus === "REJECTED" ? (
                    <div className="flex flex-col items-center justify-center py-6 gap-4 text-rose-500">
                      <div className="rounded-full bg-rose-100 p-4 dark:bg-rose-900/30">
                        <X className="h-8 w-8" />
                      </div>
                      <div className="text-2xl font-black uppercase tracking-widest">Thất bại</div>
                      {(txAdminName || order.adminNoteAuthor) && (
                    ...
                    <div className="flex flex-col items-center justify-center py-6 gap-4 text-amber-500">
                      <div className="rounded-full bg-amber-100 p-4 dark:bg-amber-900/30">
                        <Loader2 className="h-8 w-8 animate-spin" />
                      </div>
                      <div className="text-lg font-black uppercase tracking-widest">ĐANG CHỜ ĐỐI SOÁT</div>
                ...
                {paymentInfo?.latestAttempt && effectiveStatus === "PENDING" && (
                  <div className="rounded-[2rem] bg-white p-8 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-800">
                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-6 flex items-center gap-2">
                      <Shield className="h-4 w-4" /> Thao tác xử lý phê duyệt
                    </h3>
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          ...
                          className="rounded-2xl bg-indigo-600 py-3 text-xs font-black text-white hover:bg-indigo-700 transition-all disabled:opacity-50 shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                          ...
                          disabled={isProcessing}
                          className="rounded-2xl bg-rose-50 py-3 text-xs font-black text-rose-600 hover:bg-rose-100 transition-all disabled:opacity-50 border border-rose-100 flex items-center justify-center gap-2"
                        >
          ...
          {paymentInfo?.matchedTransaction && (
            <div className={`rounded-[2rem] p-8 shadow-sm ring-1 mt-6 ${
              ...
              <div className="space-y-4">
                <div className={`rounded-2xl bg-white p-4 dark:bg-slate-900 shadow-sm border ${
                  paymentInfo.matchedTransaction.reconcileStatus === "MATCHED" 
                  ...
                  <p className="text-sm font-black text-indigo-600">{paymentInfo.matchedTransaction.transactionCode}</p>
                </div>
                <div className={`rounded-2xl bg-white p-4 dark:bg-slate-900 shadow-sm border ${
                  paymentInfo.matchedTransaction.reconcileStatus === "MATCHED" 
                    ? "border-emerald-100 dark:border-emerald-500/20" 
                ...
                {paymentInfo.matchedTransaction.reconcileStatus === "MATCHED" ? (
                  <button
                    ...
                    disabled={isProcessing}
                    className="w-full rounded-2xl bg-rose-600 py-4 text-sm font-black text-white hover:bg-rose-700 transition-all duration-300 disabled:opacity-50 shadow-lg shadow-rose-500/20 flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    ...
                    disabled={isProcessing}
                    className="w-full rounded-2xl bg-indigo-600 py-4 text-sm font-black text-white hover:bg-indigo-700 transition-all duration-300 disabled:opacity-50 shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2"
                  >
