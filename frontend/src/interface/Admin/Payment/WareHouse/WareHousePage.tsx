      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <button
              onClick={() => fetchData()}
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white/10 px-3 py-2 sm:px-4 text-sm font-semibold text-white shadow-sm transition-all hover:bg-white/20 whitespace-nowrap"
            >
            ...
            {archivedAttempts.length > 0 && (
              <button
                ...
                disabled={deleting}
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-rose-600 px-3 py-2 sm:px-4 text-sm font-semibold text-white shadow-sm transition-all hover:bg-rose-700 disabled:opacity-50 whitespace-nowrap"
              >
            ...
            <Link
              href="/payments"
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-3 py-2 sm:px-4 text-sm font-semibold text-white shadow-sm transition-all hover:bg-indigo-700 whitespace-nowrap"
            >
              Quay lại
            </Link>
          </div>
      ...
      <div className="overflow-hidden rounded-[2.5rem] border border-slate-200/60 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="p-4 sm:p-8 border-b border-slate-100 dark:border-slate-800">
          <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-white flex items-center gap-2 sm:gap-3">
            <Archive className="h-5 w-5 sm:h-6 sm:w-6 shrink-0 text-amber-500" />
            <span>Bill đã lưu trữ</span>
            <span className="text-sm sm:text-base opacity-60">({archivedAttempts.length})</span>
          </h3>
        </div>
        ...
        {archivedAttempts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 sm:py-24">
            <Archive className="h-16 w-16 sm:h-20 sm:w-20 text-slate-300 dark:text-slate-600 mb-4" />
            <p className="text-base sm:text-lg font-black text-slate-400">Chưa có bill nào trong kho lưu trữ</p>
            <p className="mt-2 text-sm text-slate-500">Bill sẽ được lưu trữ khi bạn xóa vĩnh viễn đơn hàng</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                  <div className="flex items-start gap-3 sm:gap-4 min-w-0 flex-1">
                    <div className="shrink-0">
                      {attempt.transferImageUrl ? (
                        <img
                          ...
                          className="h-16 w-16 sm:h-20 sm:w-20 rounded-xl object-cover border-2 border-slate-200 dark:border-slate-700"
                          alt="Bill"
                        />
                      ) : (
                        <div className="h-16 w-16 sm:h-20 sm:w-20 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        >
                          {attempt.status === "MATCHED" ? <Check className="h-3 w-3" /> : <X className="h-3 w-3" />}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs sm:text-sm">
                        <div>
                          ...
                          <span className="font-bold text-indigo-600">{formatVnd(attempt.amount)}</span>
                        </div>
                        <div>
                          <span className="text-slate-500">Lưu trữ:</span>{" "}
                          <span className="font-bold">{formatDate(attempt.archivedAt || undefined)}</span>
                        </div>
                        <div className="sm:col-span-2">
                          <span className="text-slate-500">Ghi chú:</span>{" "}
                          <span className="italic">{attempt.transferNote || "Không có"}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-xs sm:text-sm font-bold text-white hover:bg-indigo-700 transition"
                    >
                    ...
                    <button
                      ...
                      disabled={deleting}
                      className="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-3 py-2 text-xs sm:text-sm font-bold text-white hover:bg-rose-700 transition disabled:opacity-50"
                    >
                      <Trash2 className="h-4 w-4" />
                      Xóa
                    </button>
          ...
          >
            ...
            >
              ...
              <div className="flex-1 flex flex-col overflow-hidden rounded-[2rem] sm:rounded-[3rem] lg:rounded-l-none lg:rounded-r-[3rem]">
                {/* Header */}
                <div className="flex items-center justify-between px-4 sm:px-10 py-4 sm:py-6 shrink-0"
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-10 flex flex-col">
                  ...
                  <div className="grid gap-3 sm:gap-4 grid-cols-2 mb-6 sm:mb-8">
                    <div className="p-3 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem]"
                  ...
                  <div className="mb-6 sm:mb-8 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] flex gap-3 sm:gap-4"
                    style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.2)" }}
                  >
                    <div className="p-2 sm:p-3 rounded-2xl h-fit shrink-0"
                  ...
                  <div className="mt-auto pt-6" style={{ borderTop: "1px solid rgba(255,255,255,0.1)" }}>
                    <button
                      ...
                      disabled={deleting}
                      className="w-full h-14 sm:h-16 rounded-2xl bg-rose-600 text-white font-black hover:bg-rose-700 transition-all flex items-center justify-center gap-2 shadow-xl disabled:opacity-50"
                    >
                      {deleting ? <Loader2 className="h-5 w-5 sm:h-6 sm:w-6 animate-spin" /> : <Trash2 className="h-5 w-5 sm:h-6 sm:w-6" />}
