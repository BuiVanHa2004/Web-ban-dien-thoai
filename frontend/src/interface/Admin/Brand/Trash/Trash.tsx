...
export default function Trash() {
  ...
  async function refresh() {
    setError(null);
    try {
      const trashed = await brandService.getTrash();
      setItems(
        trashed.map<TrashItem>((b) => ({
          id: String(b.brandId),
          title: b.brandName,
          subtitle: b.brandDescription || "",
          imageUrl: (b.brandImages && b.brandImages.length > 0) ? b.brandImages[0] : "",
  ...
  async function onRestore(id: string) {
    try {
      await brandService.restore(Number(id));
      await refresh();
    } catch (e: any) {
  ...
  async function onDeleteForever(id: string) {
    try {
      await brandService.deleteForever(Number(id));
      await refresh();
    } catch (e: any) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Thùng rác thương hiệu</h1>
          <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">
            Khôi phục hoặc xóa vĩnh viễn các thương hiệu đã xóa mềm.
          </p>
        </div>
        <Link
          href="/brands"
      ...
      <TrashView
        title="Thùng rác"
        description="Các thương hiệu đã bị xóa mềm sẽ nằm ở đây."
        items={items}
